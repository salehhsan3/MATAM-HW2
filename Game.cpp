...
namespace mtm
{
    ...
    void Game::attack(const GridPoint & src_coordinates, const GridPoint & dst_coordinates)
    {
        ...
        bool can_keep_attacking = false;//pass it as reference below!
                ...
                attacker->attack(main_target,src_coordinates,dst_coordinates,can_keep_attacking); 
                if ( can_keep_attacking == true )
                {
                    for ( int i = 0; i< this->height; i++)
                    {
                        for (int j = 0; j < this->width; j++)
                        {
                            if ( gameboard[i][j] == main_target )
                            {
                                continue;//already attacked him above!
                            }
                            attacker->attackNearByPlayerIfYouHaveTheAbility(gameboard[i][j],dst_coordinates);
                        }
                    }
                } 
